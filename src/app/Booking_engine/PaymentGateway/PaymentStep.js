...
const SendRequest = () => {
  const { userDetails, totalPrice, setTotalPrice, selectedPropertyId, selectedPropertyName, 
    selectedPropertyPhone } = useBookingEngineContext();
  ...
  const generateToken = async () => {
    const response = await fetch('https://staahbe.cinuniverse.com/GenerateToken', {
      ...
      body: JSON.stringify({ UserId: "clarks_pg",
      Password: "clarks@2025"  })
    });
  ...
  const generateReservationIdFromAPI = async (token) => {
    ...
    const response = await fetch('https://staahbe.cinuniverse.com/GenerateReservationId', {
      method: 'POST',
      headers: {
