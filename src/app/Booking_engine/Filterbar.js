...
const FilterBar = () => {
  ...
  const { setSelectedPropertyId, setSelectedPropertyName, setSelectedPropertyPhone, setFilteredRooms,
    selectedRoom, setSelectedRoom, setSelectedRoomRate, setPropertyId, setCancellationPolicyState 
  ...
  selectedSetRateDataList,} = useBookingEngineContext();
...
useEffect(()=>{
  ...
  useEffect(() => {
    ...
    const storedData = sessionStorage?.getItem("paymentResponse");
        if (storedData) {
          ...
          // console.log("Stored data at home page",storedData);
          // console.log("Partner id at home page",parsed.partner_id);
          // const parsed = JSON.parse(storedData);
        ...
        const fetchPropertiesFromAPI = async () => {
          //setCurrentStep(0); 
          const res = await fetch("/api/staah-api/content");
         // console.log("Fetched property list from Api res:", res);
          const data = await res?.json();
          //console.log("Fetched property list from Api json:", data);
    ...
    //     console.log("Before array", properties);
    //     if (Array.isArray(properties)) {
    //       console.log("After array", properties);
  ...
  const calculateBasePrice = () => {
    ...
    const totalRoomRate = selectedRoom?.reduce((sum, room) => sum + (room?.roomRate || 0), 0);
  ...
  const handleGetDetails = (rooms) => {
    ...
    // console.log('SeletedRoom:', selectedRoom.RackRate);
       ...
       //setSelectedRoomImage({ roomId: rooms.RoomId, roomName: rooms.RoomName });
       setSelectedRoomDetails({isPropertyVisible: true,id:selectedRoomDetails?.id}); 
     if (selectedRoomDetails?.id) {
       ...
       setSelectedRoom((prev) =>
         prev.map((room) =>
           room?.id === selectedRoomDetails?.id
             ? { ...room, roomId: rooms.RoomId, roomName: rooms.RoomName, roomRate: rooms.RackRate, roomImage: rooms.Images[0] }
             : room
         )
       );
     } else {
       ...
       setSelectedRoom((prev) => {
        console.log("setSelectedRoomprev", prev);
         if (prev.length > 0) {
          const first = prev[0];
          console.log("setSelectedRoomprev1", prev);
           return [
             { roomId: rooms.RoomId, roomName: rooms.RoomName, roomRate: rooms.RackRate, roomImage: rooms.Images[0], adults: first?.adults, children: first?.children },
             ...prev.slice(1) // Keep the rest unchanged
           ];
         } else {
          console.log("setSelectedRoomprev2", prev);
           return [{ roomId: rooms.RoomId, roomName: rooms.RoomName, roomRate: rooms.RackRate, roomImage: rooms.Images[0], adults: 1, children: 0 }];
         }
       });
     }
       setSelectedRoomRate({ roomId: rooms.RoomId, roomRate: rooms.RackRate });
       setTimeout(() => {
  ...
  const handleSelectRoom =(mapping,rate)=>{
    SelectedRoomWithOffer(mapping);   
      if (selectedRoomDetails?.id) {
        ...
        setSelectedRoom((prev) =>
          prev.map((room) =>
            room?.id === selectedRoomDetails?.id
              ? { ...room, roomPackage: rate.RateName, rateId: rate.RateId}
              : room
          )
        );
      } else {
        ...
        setSelectedRoom((prev) => {
          if (prev.length > 0) {
            return [
              { ...prev[0], roomPackage: rate.RateName, rateId: rate.RateId }, 
              ...prev.slice(1)
            ];
          } else {
  ...
  const showBookingEngine = () => {
    ...
    // Scroll smoothly to booking form after visibility change
    setTimeout(() => {
      bookingRef.current?.scrollIntoView({ behavior: "smooth" });
    }, 100); // delay allows time for the class to apply
  };
...
console.log(selectedRoom);
  ...
  return (
    <>
          ...
          <div className="banner-search-btn">
            {!isVisible && (
              <a className="banner-search-btn-link" onClick={showBookingEngine} id="booking-engine-btn">
                <Image src="/images/search-icon-white.svg" height={100} width={100} alt="search icon" />
              </a>
            )}
          </div>
        </div>
      ...
      <section
  ref={bookingRef}
  className={`booking-form-wrapper toggle-div ${isVisible ? "visible" : "hidden"}`}>
                                ...
                                <div className="col-md-3">
                                  ...
                                  <div className="book-a-stay d-flex justify-content-end">
                                    <button className="btn btn-primary accordion-button" onClick={() => {
                                      ...
                                      const propertyName = property.PropertyData.PropertyName;
                                      const propertyPhone = property.PropertyData.Address.Phone;
                                      setSelectedPropertyPhone(propertyPhone);
                                          ...
                                          <div className="col-md-3">
                                            ...
                                            <div className="book-a-stay d-flex justify-content-end">
                                              <button
                                                ...
                                                //     if (prev.length > 0) {
                                                //       return [
                                                //         { roomId: rooms.RoomId, roomName: rooms.RoomName, roomRate: rooms.RackRate, roomImage: rooms.Images[0] },
                                                //         ...prev.slice(1) // Keep the rest unchanged
                                                //       ];
                                                //     } else {
                                                //       return [{ roomId: rooms.RoomId, roomName: rooms.RoomName, roomRate: rooms.RackRate, roomImage: rooms.Images[0] }];
                                                //     }
                                                //   });
                                                // }
                                                //   setSelectedRoomRate({ roomId: rooms.RoomId, roomRate: rooms.RackRate });
                                                      ...
                                                      <div className="price-details">
                                                        ...
                                                        <div>
                                                          ...
                                                          <div className="select-button book-a-stay">
                                                             ...
                                                             {handleSelectRoom(mapping,rate); selectedSetRateDataList(rate);}
                                                             }> 
                                                             Select </button>
